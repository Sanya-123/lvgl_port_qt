...
static lv_style_t styleButtons;
...
#define CREATE_BUTTON(btn, label, x) do{ btn = lv_btn_create(lv_scr_act());   \
                                        ...
                                        lv_obj_set_size(btn, LV_HOR_RES/4, BUTTON_HEIGHT);  \
                                        lv_obj_set_pos(btn, LV_HOR_RES/4*x, LV_VER_RES - BUTTON_HEIGHT);}while(0);

/**********************
 *   GLOBAL FUNCTIONS
 **********************/

void lv_demo_meteo(void)
{
    ...
    lv_style_set_border_width(&styleButtons, 0);
    ...
    lv_obj_set_size(tl, LV_HOR_RES, LV_VER_RES - BUTTON_HEIGHT);
    ...
    lv_obj_add_event_cb(buttonLeft, btn_event_cb, LV_EVENT_ALL, &typeLeft);
    lv_obj_add_event_cb(buttonRight, btn_event_cb, LV_EVENT_ALL, &typeRight);


    //windows
    /*Tile1: just a label*/
    lv_obj_t * tile1 = lv_tileview_add_tile(tl, 0, 0, LV_DIR_RIGHT);
    lv_obj_t * label1 = lv_label_create(tile1);
    lv_label_set_text(label1, "Scroll RIGHT");
    lv_obj_center(label1);

    /*Tile1: just a label*/
    lv_obj_t * tile2 = lv_tileview_add_tile(tl, 1, 0, LV_DIR_LEFT | LV_DIR_RIGHT);
    lv_obj_t * label2 = lv_label_create(tile2);
    lv_label_set_text(label2, "Scroll RIGHT | LEFT");
    lv_obj_center(label2);

    /*Tile1: just a label*/
    lv_obj_t * tile3 = lv_tileview_add_tile(tl, 2, 0, LV_DIR_LEFT);
...
static void btn_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        lv_obj_set_tile_id(tl, numDisplay, 0, LV_ANIM_ON);
